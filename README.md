...
One exciting improvement would be to use spline fits of the previous bounding box trajectories for each track to better estimate the "cost" of assigning a new detection at the end of that trajectory. If the track was already on its way to the new_box location (like a car slowly pulling forward), it shouldn't matter far away it WAS, it should matter how much it had to "accelerate" between its last known {location, speed} to get to the proposed new box location.  It would also be interesting to use nearest neighbors on the features of detected cars to keep track of car IDENTITY so that the tracker can recover after occlusions.  The distance in feature space could be used as an additional imput to the cost matrix in the Hungarian assignment algorithm.
